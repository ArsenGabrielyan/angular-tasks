...
id: string = localStorage.getItem("to-do-tab") || "active";
  ...
  tabChange(t:string){this.id = t;localStorage.setItem("to-do-tab", this.id)}
  addToDo(form: NgForm){
  ...
  deleteToDo(i:number){
    ...
    if(sure){this.completed.splice(i,1);localStorage.setItem("to-do-completed", JSON.stringify(this.completed))}
  }
  markAll(){
  ...
  removeItem(parent: any, i:number, count: number = 1){
    ...
    localStorage.setItem("to-do-completed", JSON.stringify(this.completed))
  }
  clearAll(){
    ...
    this.completed.map(()=>{if(sure){this.completed.splice(0,this.completed.length);localStorage.setItem("to-do-completed", JSON.stringify(this.completed))}})
  }
}
